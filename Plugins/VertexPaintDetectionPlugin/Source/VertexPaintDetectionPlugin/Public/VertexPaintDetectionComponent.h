...
public:
	...
	virtual ~UVertexPaintDetectionComponent() override;
	...
	bool IsPaintSettingsSetToApplyAnyColor(const FVertexPaintColorSettingStruct& colorSettings);

	void SetPaintConditionLineOfSightToActorLocation(FVertexPaintColorSettingStruct& paintColorSettings);


	//--------------------------------------------------
	// Vertex Paint / Detection Settings

	UPROPERTY(BlueprintAssignable, Category = "Runtime Vertex Color Paint and Detection Plugin|Detection", meta = (ToolTip = "Broadcasts when Finished Getting Vertex Data, either successfully or unsuccessfully."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin|Painting", meta = (ToolTip = "If you only want this component to be able to paint on certain actors then add them here. \nUseful if you for instance have a faucet that drips and paints out puddles of water etc. with sphere traces but doesn't want the player to get painted as well if they walk into the sphere but only the floor. "))
		TArray<TSoftObjectPtr<AActor>> canOnlyApplyPaintOnTheseActors;
...
private:
	...
	int maxAllowedTasksPerMesh = 100;
};
