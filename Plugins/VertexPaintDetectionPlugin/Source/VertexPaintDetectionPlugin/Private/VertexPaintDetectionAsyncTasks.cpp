...
void FVertexPaintDetectionAsyncTasks::DoWork() {
	...
	if (IsValid(calculateColorsInfo_Global.paintOnMeshSettings.actor)) {
	...
	if (IsValid(calculateColorsInfo_Global.getAllVertexColorsSettings.actor)) {
		...
		amountOfVerticesAtEachLOD_Global.SetNum(calculateColorsInfo_Global.lodsToLoopThrough, true);
		...
		for (int i = 0; i < calculateColorsInfo_Global.lodsToLoopThrough; i++) {
			...
			amountOfVerticesAtEachLOD_Global[i] = vertexDetectMeshData_Global.meshDataPerLOD[i].meshVertexColorsPerLODArray.Num();
		...
		if (vertexDetectMeshData_Global.meshDataPerLOD.IsValidIndex(0) && vertexDetectMeshData_Global.meshDataPerLOD[0].meshVertexColorsPerLODArray.Num() > 0) {
			...
			paintResultedInChange_Global = true;
		...
		FString rapidJsonStringToDeSerialize;

		if (calculateColorsInfo_Global.setMeshComponentVertexColorsUsingSerializedStringSettings.actor) {

			rapidJsonStringToDeSerialize = calculateColorsInfo_Global.setMeshComponentVertexColorsUsingSerializedStringSettings.serializedColorDataAtLOD0;
		}

		else if (calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetDataAssetSnippetIsStoredIn->snippetColorData.Contains(calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetID)) {

			if (vertexDetectMeshData_Global.meshDataPerLOD.IsValidIndex(0) && calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetDataAssetSnippetIsStoredIn->snippetColorData.FindRef(calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetID).colorSnippetPerLODAsRapidJSon.IsValidIndex(0)) {

				rapidJsonStringToDeSerialize = calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetDataAssetSnippetIsStoredIn->snippetColorData.FindRef(calculateColorsInfo_Global.paintOnColorSnippetSettings.colorSnippetID).colorSnippetPerLODAsRapidJSon[0].colorsAtLODAsJSonString;
			}
		}
		...
		if (rapidJsonStringToDeSerialize.Len() > 0) {
			...
			// Only bothers with LOD0 for snippets and set mesh component from serialized array, they have the option to propogate the LOD0 to the rest of the LODs
			if (!vertexDetectMeshData_Global.meshDataPerLOD.IsValidIndex(0))
				vertexDetectMeshData_Global.meshDataPerLOD.SetNum(1, false);


			TArray<FColor> colorsFromLOD_Local = vertexDetectMeshData_Global.meshDataPerLOD[0].meshVertexColorsPerLODArray;
			int amountOfVertices_Local = 0;

			if (colorsFromLOD_Local.Num() <= 0)
				colorsFromLOD_Local = GetVertexColorsAtLOD(0, calculateColorsInfo_Global.vertexPaintComponent);

			amountOfVertices_Local = colorsFromLOD_Local.Num();


			// Gets the total one which is the one we use to check if we've fully painted or fully removed a mesh etc. so we don't override vertex colors if there's not gonna be any difference
			amountOfPaintedColorsOfEachChannelAbove0_Global = UVertexPaintFunctionLibrary::GetAmountOfPaintedColorsForEachChannel(colorsFromLOD_Local, 0);
			amountOfVerticesAtEachLOD_Global.SetNum(calculateColorsInfo_Global.lodsToLoopThrough, true);


			// De-Serializes the colors to apply
			TArray<FColor> deserializedLOD0Color_Local = URapidJsonFunctionLibrary::DeserializeTArrayFColor_Wrapper(rapidJsonStringToDeSerialize);


			// If the De-serialized Colors at LOD0 matches the original LOD0 colors amount
			if (deserializedLOD0Color_Local.Num() == amountOfVertices_Local) {

				newColorVertexBufferPerLOD_Global[0] = new FColorVertexBuffer();
				newColorVertexBufferPerLOD_Global[0]->InitFromColorArray(deserializedLOD0Color_Local);
				amountOfVerticesAtEachLOD_Global[0] = amountOfVertices_Local;

				vertexDetectMeshData_Global.meshDataPerLOD[0].lod = 0;
				vertexDetectMeshData_Global.meshDataPerLOD[0].meshVertexColorsPerLODArray = deserializedLOD0Color_Local;;
				...
				calculateColorsResultMessage.Add(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet / Set Mesh Colors with Serialized String Task Fail as the De-Serialized Colors didn't match the amount of Vertices the Mesh has at LOD0! Mesh LOD0 Vertex Amount: %i   -  Deserialized LOD0 Amount: %i"), amountOfVertices_Local, deserializedLOD0Color_Local.Num()));
			}
		}

		else {

			calculateColorsResultMessage.Add(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet / Set Mesh Colors with Serialized String Task Fail as the String to De-Serialize and get colors from is 0 in Length!")));
		}
	}
		...
		if (!calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh && calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyVertexColorSetting == EApplyVertexColorSetting::ESetVertexColor) {
			...
			// If there's no reason to loop through all the vertices then initializes the color buffer right away. 
			if (!calculateColorsInfo_Global.paintOnEntireMeshSettings.limitPaintColor.usePaintLimits && !calculateColorsInfo_Global.paintOnEntireMeshSettings.limitPaintToSpecifiedNormal.limitPaintingToSpecifiedWorldNormal && !IsThereAnyPaintConditions(calculateColorsInfo_Global.paintOnEntireMeshSettings.applyVertexColorSettings) && !ShouldLoopThroughVerticesAndLODs(calculateColorsInfo_Global.fundementalStruct.callbackSettings, calculateColorsInfo_Global.vertexPaintComponent)) {
				...
				for (int i = 0; i < calculateColorsInfo_Global.lodsToLoopThrough; i++) {
					...
					amountOfVerticesAtEachLOD_Global[i] = meshLODTotalAmountOfVerts_Local;
	...
	calculateColorsInfo_Global.triedToApplyColorOnAnyVertex = triedToApplyColorOnAnyVertex_Global;
	calculateColorsInfo_Global.amountOfVertexPositionsAtEachLOD = amountOfVertexPositionsAtEachLOD_Global;
	calculateColorsInfo_Global.amountOfVerticesAtEachLOD = amountOfVerticesAtEachLOD_Global;
	...
	calculateColorsInfo_Global.amountOfPaintedColorsOfEachChannelAtMinAmount = amountOfPaintedColorsOfEachChannelAtMinAmount_Global;
	...
	// Cloth
	if (calculateColorsInfo_Global.paintOnMeshSettings.affectClothPhysics)
	...
	if (IsInGameThread()) {
		...
		if (UVertexPaintFunctionLibrary::IsWorldValid(calculateColorsInfo_Global.worldTaskWasCreatedIn)) {
			...
			if (IsValid(gameInstance_Local)) {
				...
				if (IsValid(gameInstanceSubsystem_Local)) {
					...
					gameInstanceSubsystem_Local->CalculateColorsTaskFinished(calculateColorsInfo_Global, paintTasResultTemp, calculateColorsResultMessage, paintResultedInChange_Global);
				}
			}
		}
	}
		...
		bool paintResultedInChangeTemp = paintResultedInChange_Global;

		// Broadcasts on Game Thread so the actual Applying of colors happens there
		AsyncTask(ENamedThreads::GameThread, [calculateColorsInfoTemp, paintTasResultTemp, resultMessageTemp, paintResultedInChangeTemp]() {
			...
			if (UVertexPaintFunctionLibrary::IsWorldValid(calculateColorsInfoTemp.worldTaskWasCreatedIn)) {
				...
				if (IsValid(gameInstance_Local)) {
					...
					if (IsValid(gameInstanceSubsystem_Local)) {
						...
						gameInstanceSubsystem_Local->CalculateColorsTaskFinished(calculateColorsInfoTemp, paintTasResultTemp, resultMessageTemp, paintResultedInChangeTemp);
					}
				}
			}
			});
...
EVertexPaintDetectionTaskResult FVertexPaintDetectionAsyncTasks::CalculateColorToApply(bool updateMeshVertexColorData) {
	...
	TArray<int> paintEntireMeshRandom_VerticesRandomizedAtLOD0_Local;
	...
	if (calculateColorsInfo_Global.vertexPaintSkelComponent) {
		...
		if (calculateColorsInfo_Global.vertexPaintSkelComponent->GetSkeletalMeshRenderData()->LODRenderData[calculateColorsInfo_Global.vertexPaintSkelComponent->GetPredictedLODLevel()].HasClothData()) {
			...
			if (calculateColorsInfo_Global.paintOnMeshSettings.affectClothPhysics && UKismetSystemLibrary::DoesImplementInterface(calculateColorsInfo_Global.paintOnMeshSettings.actor, UVertexPaintDetectionInterface::StaticClass())) {

				clothsToColorAndPhysicsSettings_Local = IVertexPaintDetectionInterface::Execute_GetSkeletalMeshClothPhysicsSettings(calculateColorsInfo_Global.paintOnMeshSettings.actor, calculateColorsInfo_Global.vertexPaintSkelComponent);
			}
			...
			if (compsToCheckWithinTemp.paintWithinAreaShape == Enum_PaintWithinAreaShape::isComplexShape) {
				...
				if (compsToCheckWithinTemp.componentToCheckIfIsWithin_IsSkeletalMeshComponent) {
					...
					if (auto paintWithinArea_skelMeshComp_Local = Cast<USkeletalMeshComponent>(compsToCheckWithinTemp.componentToCheckIfIsWithin)) {
						...
						if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentCenter) {
						...
						else if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentZHeight) {


							paintWithinArea_AreaOfEffects_Local[i] = compsToCheckWithinTemp.componentToCheckIfIsWithin_Bounds.GetBox().GetExtent().Z * calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin->GetComponentScale().Z;
						}
					}
				}
					...
					if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentCenter) {
						...
						paintWithinArea_AreaOfEffects_Local[i] = calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin_CleanAggGeomBounds.GetBox().GetExtent().Z * calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin_WorldTransform.GetScale3D().Z;
					}
				}
					...
					if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentCenter) {
						...
						paintWithinArea_AreaOfEffects_Local[i] = compsToCheckWithinTemp.componentToCheckIfIsWithin_ActorBounds_Extent.Z;
					}
				}
			}
				...
				if (calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin_IsBoxComponent) {
					...
					if (compsToCheckWithinTemp.paintWithinAreaShape == Enum_PaintWithinAreaShape::isSquareOrRectangleShape) {
						...
						if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentCenter) {
							...
							paintWithinArea_AreaOfEffects_Local[i] = calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin_ScaledBoxExtent.Z;
						}
					}
				}
	...
	amountOfVertexPositionsAtEachLOD_Global.SetNum(lodsToLoopThrough_Local, false);
	amountOfVerticesAtEachLOD_Global.SetNum(lodsToLoopThrough_Local, false);
	...
	for (int currentLOD_Local = 0; currentLOD_Local < lodsToLoopThrough_Local; currentLOD_Local++) {
		...
		if (IsValid(calculateColorsInfo_Global.vertexPaintStaticMeshComponent)) {
			...
			// If static mesh then there are no sections so just adds 0 so we loop through it once
			sectionsToLoopThrough_Local.Add(0);

			// if cpu allowed mesh they don't store vertex pos and normal as it is accessible in runtime, so we get them from the buffer
			...
			// Sets the loop amount to be this LODs vert amount
			sectionMaxAmountOfVerts_Local = posVertBufferAtLOD_Local->GetNumVertices();
		...
		amountOfVerticesAtEachLOD_Global[currentLOD_Local] = meshLODTotalAmountOfVerts_Local;
		...
		for (auto currentSection_Local : sectionsToLoopThrough_Local) {
			...
			for (uint32 currentSectionVertex_Local = 0; currentSectionVertex_Local < (uint32)sectionMaxAmountOfVerts_Local; currentSectionVertex_Local++) {
					...
					if (skelMeshRenderData_Local.RenderSections[currentSection_Local].HasClothingData() && currentLOD_Local == calculateColorsInfo_Global.vertexPaintSkelComponent->GetPredictedLODLevel()) {
						...
						if (currentClothSectionVertexPositionsTemp_Local.IsValidIndex(currentSectionVertex_Local) && currentClothSectionVertexNormalsTemp_Local.IsValidIndex(currentSectionVertex_Local)) {
							...
							*/
							...
							if (calculateColorsInfo_Global.drawClothVertexPositionDebugPoint && !calculateColorsInfo_Global.multiThreadSettings.useMultithreadingForCalculations) {
				...
				if (calculateColorsInfo_Global.paintOnEntireMeshSettings.actor) {
					...
					if (calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh) {
						...
						// AmountOfVerticesLeftToRandomize_Local will reset if Not to propogate to other LODs, i.e. do a complete randomization for each LOD
							...
							if (UKismetMathLibrary::RandomBoolWithWeightFromStream(paintEntireMesh_Random_Probability_Local, paintEntireMesh_Random_Stream_Local)) {
								...
								paintEntireMeshRandom_VerticesRandomizedAtLOD0_Local.Add(currentLODVertex_Local);
					...
					// If paint entire mesh but not set to randomize
					else {
				...
				if (calculateColorsInfo_Global.drawVertexNormalDebugArrow && !calculateColorsInfo_Global.multiThreadSettings.useMultithreadingForCalculations)
				...
				if (calculateColorsInfo_Global.drawVertexPositionDebugPoint && !calculateColorsInfo_Global.multiThreadSettings.useMultithreadingForCalculations)
						...
						if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor) {
							...
							if (calculateColorsInfo_Global.estimatedColorAtHitLocationSettings.getEstimatedColorAtHitLocation) {
								...
								if (calculateColorsInfo_Global.estimatedColorAtHitLocationSettings.onlyGetIfMeshHasMaxAmountOfVertices) {
									...
									if (amountOfVerticesAtEachLOD_Global[0] <= calculateColorsInfo_Global.estimatedColorAtHitLocationSettings.maxAmountOfVertices)
				...
				if (calculateColorsInfo_Global.paintOnMeshSettings.actor && colorFromLOD_Local.IsValidIndex(currentLODVertex_Local)) {


					// Checks distances etc. if painting at location and within area and updated color array

					vertexColorGotChanged_Local = false;

					if (WillAppliedColorMakeChangeOnVertex(colorFromLOD_Local[currentLODVertex_Local], normalsFromLOD_Local[currentLODVertex_Local], calculateColorsInfo_Global.paintOnMeshColorSettings)) {
						...
						if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor && calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffect > 0) {
								...
								if (IsVertexWithinArea(currentVertexWorldSpace, calculateColorsInfo_Global.paintOnMeshWithinAreaSettings, calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i])) {
									...
									if (calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].paintWithinAreaShape == Enum_PaintWithinAreaShape::isComplexShape) {
										...
										if (calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.falloffType == EVertexPaintFallOffType::FromComponentCenter)
											...
											paintWithinArea_DistanceFromVertex_Local = currentVertexWorldSpace.Z - calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i].componentToCheckIfIsWithin_ActorBounds_Origin.Z;
									}
							...
							if (paintEntireMesh_PaintCurrentVertex_Local) {
								...
								colorFromLOD_Local[currentLODVertex_Local] = GetColorToApplyOnVertex(colorFromLOD_Local[currentLODVertex_Local], currentVertexWorldSpace, normalsFromLOD_Local[currentLODVertex_Local], currentBoneName_Local, 0, 0, 0, 0, calculateColorsInfo_Global.paintOnMeshColorSettings, vertexColorGotChanged_Local);
						...
						if (triedToApplyColorOnVertex_Local) {
							...
							if (calculateColorsInfo_Global.drawVertexPositionDebugPointOnVertexThatGotPaintApplied && !calculateColorsInfo_Global.multiThreadSettings.useMultithreadingForCalculations)
						...
						if (!triedToApplyColorOnAnyVertex_Global)
							triedToApplyColorOnAnyVertex_Global = triedToApplyColorOnVertex_Local;
					}

					if (!paintResultedInChange_Global)
						paintResultedInChange_Global = vertexColorGotChanged_Local;

					// Updates color buffer vertex color, so when we apply the colors we can just set to use the new buffer and don't have to run InitFromColorArray which loops through all of the verts 
					if (newColorVertexBufferPerLOD_Global.IsValidIndex(currentLOD_Local)) {
				...
				if (calculateColorsInfo_Global.paintOnMeshSettings.affectClothPhysics && calculateColorsInfo_Global.vertexPaintSkelComponent && clothsToColorAndPhysicsSettings_Local.Num() > 0 && currentLOD_Local == calculateColorsInfo_Global.vertexPaintSkelComponent->GetPredictedLODLevel()) {
		...
		if (currentLOD_Local == 0) {
			...
			if (closestVertexFound_Paint_Global >= 0) {
				...
				if (calculateColorsInfo_Global.vertexPaintStaticMeshComponent && calculateColorsInfo_Global.paintOnMeshAtLocationSettings.hitFundementals.optionalFaceIndexForStaticMeshComponents >= 0) {

					int sectionIndexOfCollisionFaceIndex_Local = 0;

					materialAtClosestPaintVertex_Global = calculateColorsInfo_Global.vertexPaintStaticMeshComponent->GetMaterialFromCollisionFaceIndex(calculateColorsInfo_Global.paintOnMeshAtLocationSettings.hitFundementals.optionalFaceIndexForStaticMeshComponents, sectionIndexOfCollisionFaceIndex_Local);
				}
			...
			if (closestVertexFound_Detection_Global >= 0) {
				...
				// Getting closest materials
				if (calculateColorsInfo_Global.vertexPaintStaticMeshComponent && calculateColorsInfo_Global.detectInfoSettings.hitFundementals.optionalFaceIndexForStaticMeshComponents >= 0) {

					int sectionIndexOfCollisionFaceIndex_Local = 0;

					materialAtClosestDetectVertex_Global = calculateColorsInfo_Global.vertexPaintStaticMeshComponent->GetMaterialFromCollisionFaceIndex(calculateColorsInfo_Global.detectInfoSettings.hitFundementals.optionalFaceIndexForStaticMeshComponents, sectionIndexOfCollisionFaceIndex_Local);
				}
		...
		amountOfVertexPositionsAtEachLOD_Global[currentLOD_Local] = positionsInWorldSpaceFromLOD_Local.Num();
		...
		if (calculateColorsInfo_Global.vertexPaintSkelComponent) {
			...
			if (calculateColorsInfo_Global.paintOnMeshSettings.affectClothPhysics && paintResultedInChange_Global && clothPaintedColors_Local.Num() > 0 && paintedCloths_Local.Num() > 0 && currentLOD_Local == calculateColorsInfo_Global.vertexPaintSkelComponent->GetPredictedLODLevel()) {
	...
	if (updateMeshVertexColorData && vertexDetectMeshData_Global.meshDataPerLOD.Num() > 1 && calculateColorsInfo_Global.propogateLOD0ToAllLODs) {
		...
		vertexDetectMeshData_Global = UVertexPaintFunctionLibrary::PropogateLOD0VertexColorsToLODs(paintedVerticesAtLOD0_Local, vertexDetectMeshData_Global, completeLODsBaseBounds_Local, newColorVertexBufferPerLOD_Global);
	}
...
bool FVertexPaintDetectionAsyncTasks::ShouldLoopThroughVerticesAndLODs(FVertexDetectEventSettings callbackSettings, UPrimitiveComponent* component) {
	...
	if (callbackSettings.includeSerializedVertexColorData || callbackSettings.includeVertexPositionData || callbackSettings.includeVertexNormalData || (callbackSettings.includeColorsOfEachBone && Cast<USkeletalMeshComponent>(component)))
		return true;
...
FVector FVertexPaintDetectionAsyncTasks::Modified_GetTypedSkinnedVertexPosition(const USkinnedMeshComponent * SkinnedComp, const FSkelMeshRenderSection & Section, const FPositionVertexBuffer & PositionVertexBuffer, const FSkinWeightVertexBuffer & SkinWeightVertexBuffer, const int32 VertIndex, const TArray<FMatrix44f>&RefToLocals, uint32 & boneIndex) {
...
#endif
	{
		...
		int32 meshBoneIndex_Local = 0;

		if (Section.BoneMap.IsValidIndex(SkinWeightVertexBuffer.GetBoneIndex(BufferVertIndex, InfluenceIndex)))
			meshBoneIndex_Local = Section.BoneMap[SkinWeightVertexBuffer.GetBoneIndex(BufferVertIndex, InfluenceIndex)];

		if (InfluenceIndex == 0)
			boneIndex = meshBoneIndex_Local;

		int32 TransformBoneIndex = meshBoneIndex_Local;
		...
		if (MasterPoseComponentInst) {
			...
			TransformBoneIndex = MasterBoneMap[meshBoneIndex_Local];
		}
		...
		if (BufferVertIndex < (int32)PositionVertexBuffer.GetNumVertices() && meshBoneIndex_Local < RefToLocals.Num())
			SkinnedPos += (FVector)RefToLocals[meshBoneIndex_Local].TransformPosition(PositionVertexBuffer.VertexPosition(BufferVertIndex)) * (float)(SkinWeightVertexBuffer.GetBoneWeight(BufferVertIndex, InfluenceIndex) / 255.0f);
	}
...
bool FVertexPaintDetectionAsyncTasks::IsVertexWithinArea(const FVector & vertexWorldPosition, const FVertexPainthWithinAreaStruct & vertexPaintWithinAreaStruct, const FVertexPainthWithinArea_ComponentToCheckIfIsWithinInfo & componentToCheckIfIsWithinInfo) {
	...
	case Enum_PaintWithinAreaShape::isSquareOrRectangleShape:
		...
		isVertexWithinArea_SquareXSize_Global = componentToCheckIfIsWithinInfo.componentToCheckIfIsWithin_IsVertexWithinArea_CubeOrRectangle_SquareXSize + vertexPaintWithinAreaStruct.extraExtentToApplyPaintOn;
		isVertexWithinArea_SquareYSize_Global = componentToCheckIfIsWithinInfo.componentToCheckIfIsWithin_IsVertexWithinArea_CubeOrRectangle_SquareYSize + vertexPaintWithinAreaStruct.extraExtentToApplyPaintOn;
		isVertexWithinArea_SquareZSize_Global = componentToCheckIfIsWithinInfo.componentToCheckIfIsWithin_IsVertexWithinArea_CubeOrRectangle_SquareZSize + vertexPaintWithinAreaStruct.extraExtentToApplyPaintOn;
...
bool FVertexPaintDetectionAsyncTasks::IsCurrentVertexWithinPaintColorConditions(const FColor & currentVertexColor, const FVector & currentVertexPosition, const FVector & currentVertexNormal, const FName & currentBoneName, const FVertexDetectPaintColorConditionStruct & paintConditions, bool& applyColorStrengthIfConditionIsNotMet, float& colorStrengthIfConditionsIsNotMet) {
	...
	for (auto withinLineOfSightCondition : paintConditions.ifVertexHasLineOfSightTo) {
		...
		if (calculateColorsInfo_Global.drawPaintConditionsDebugSymbols && !calculateColorsInfo_Global.multiThreadSettings.useMultithreadingForCalculations)
...
bool FVertexPaintDetectionAsyncTasks::WillAppliedColorMakeChangeOnVertex(const FColor & currentColorOnVertex, const FVector & currentNormalOnVertex, const FVertexPaintColorSettingStruct & paintColorSettings) {


	if (paintColorSettings.limitPaintToSpecifiedNormal.limitPaintingToSpecifiedWorldNormal) {

		// Normal can't be zero, if so then it means we weren't able to get it
		if (currentNormalOnVertex.IsZero()) {

			return false;
		}

		if (currentNormalOnVertex.Equals(paintColorSettings.limitPaintToSpecifiedNormal.worldNormal, paintColorSettings.limitPaintToSpecifiedNormal.errorTolerance)) {
