...
void UVertexPaintDetectionEdSubsystem::SetEngineSpecificButtonSettings(TArray<UButton*> buttons, TArray<UComboBoxString*> comboBoxes) {
	...
	for (auto buttonTemp : buttons) {
		...
		buttonTemp->WidgetStyle.Normal.TintColor = FSlateColor(FLinearColor::White);
		buttonTemp->WidgetStyle.Normal.OutlineSettings.Width = 0;

		buttonTemp->WidgetStyle.Hovered.TintColor = FSlateColor(FLinearColor::White);
		buttonTemp->WidgetStyle.Hovered.OutlineSettings.Width = 0;

		buttonTemp->WidgetStyle.Pressed.TintColor = FSlateColor(FLinearColor::White);
		buttonTemp->WidgetStyle.Pressed.OutlineSettings.Width = 0;

		buttonTemp->WidgetStyle.Disabled.TintColor = FSlateColor(FLinearColor::White);
		buttonTemp->WidgetStyle.Disabled.OutlineSettings.Width = 0;
		buttonTemp->WidgetStyle.Disabled.DrawAs = ESlateBrushDrawType::NoDrawType;
	}

	for (auto comboBoxTemp : comboBoxes) {

		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Normal.DrawAs = ESlateBrushDrawType::Box;
		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Normal.OutlineSettings.Width = 0;

		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Hovered.DrawAs = ESlateBrushDrawType::Box;
		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Hovered.OutlineSettings.Width = 0;

		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Pressed.DrawAs = ESlateBrushDrawType::Box;
		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Pressed.OutlineSettings.Width = 0;

		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Disabled.DrawAs = ESlateBrushDrawType::Box;
		comboBoxTemp->WidgetStyle.ComboButtonStyle.ButtonStyle.Disabled.OutlineSettings.Width = 0;
	}
...
void UVertexPaintDetectionEdSubsystem::AddMeshColorSnippet(UMeshComponent* meshComponent, FString colorSnippetID, UVertexPaintColorSnippetDataAsset* colorSnippetDataAsset, bool& success) {
	...
	if (auto staticMeshComponent_Local = Cast<UStaticMeshComponent>(meshComponent)) {
		...
		FVertexDetectSerializedColorsPerLODStruct serializedColorPerLODTemp;
		serializedColorPerLODTemp.lod = 0;
		serializedColorPerLODTemp.colorsAtLODAsJSonString = URapidJsonFunctionLibrary::SerializeTArrayFColor_Wrapper(UVertexPaintFunctionLibrary::VertexPaintDetectionPlugin_GetStaticMeshVertexColorsAtLOD(staticMeshComponent_Local, 0));

		colorSnippetDataTemp.colorSnippetPerLODAsRapidJSon.Add(serializedColorPerLODTemp);
		...
		FVertexDetectSerializedColorsPerLODStruct serializedColorPerLODTemp;
		serializedColorPerLODTemp.lod = 0;
		serializedColorPerLODTemp.colorsAtLODAsJSonString = URapidJsonFunctionLibrary::SerializeTArrayFColor_Wrapper(UVertexPaintFunctionLibrary::UVertexPaintFunctionLibrary::VertexPaintDetectionPlugin_GetSkeletalMeshVertexColorsAtLOD(skeletalMeshComponent_Local, 0));
...
// Refresh Color Snippet As Rapid Json String

void UVertexPaintDetectionEdSubsystem::RefreshColorSnippetAsRapidJsonString(UVertexPaintColorSnippetDataAsset* colorSnippetDataAsset, FString colorSnippet) {
	...
	for (auto& snippetColorData_Local : colorSnippetDataAsset->snippetColorData) {
		...
		if (colorSnippet == snippetColorData_Local.Key) {
			...
			if (snippetColorData_Local.Value.colorSnippetDataPerLOD.IsValidIndex(0)) {
				...
				FVertexDetectSerializedColorsPerLODStruct serializedColorPerLODTemp;
				serializedColorPerLODTemp.lod = 0;
				serializedColorPerLODTemp.colorsAtLODAsJSonString = URapidJsonFunctionLibrary::SerializeTArrayFColor_Wrapper(snippetColorData_Local.Value.colorSnippetDataPerLOD[0].meshVertexColorsPerLODArray);
...
// Get All Actors from Level

TArray<AActor*> UVertexPaintDetectionEdSubsystem::GetAllActorsFromLevel(TSoftObjectPtr<UWorld> level) {

	TArray<AActor*> foundActors_Local;

	if (!level.Get()) return foundActors_Local;

	// UE_LOG(LogTemp, Warning, TEXT("Runtime Vertex Color Paint and Detection Plugin - GetAllActorsFromLevel()"));


	// If Persistent level
	if (level.Get() == level.Get()->PersistentLevel->GetWorld()) {

		foundActors_Local = level.Get()->PersistentLevel->Actors;
	}

	// Else checks the Streaming Level and gets the actors from that
	else {

		for (auto streamingLevel_Local : level.Get()->PersistentLevel->GetWorld()->GetStreamingLevels()) {

			if (!streamingLevel_Local->GetLoadedLevel()) continue;

			// Loops through Streaming Sub Levels until it gets to the one that the Manager is in
			if (streamingLevel_Local->GetLoadedLevel()->GetOuter() == level) {

				foundActors_Local = streamingLevel_Local->GetLoadedLevel()->Actors;
